...
>[!info] Using Twilio and request libraries

## Defining the stock information that is required
```python
STOCK_NAME = "TSLA"  
...
```
## Setup API Keys and Auth tokens
```python
...
## Using `request` to get the json and using it
```python
...
```
## Calculating the difference for sending the message
```python
...
```
## Fetching news articles that is relevant to the stock if there is a big difference
```python
...
```
## Using Twilio library to send a custom message with the articles
```python
